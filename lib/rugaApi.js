...
const brainly = async (query) => new Promise((resolve, reject) => {
    axios.get(`https://zekais-api.herokuapp.com/brainly?query=${query}&apikey=${zekais}`)
        .then((res) => {
            resolve(res.data)
        })
        .catch((err) => {
...
const cersex = async () => new Promise((resolve, reject) => {
    axios.get(`https://zenzapi.xyz/api/cersex?apikey=${zenzapi}`)
        .then((res) => {
            resolve(res.data.result)
        })
        .catch((err) => {
...
const jadwaldaerah = (daerah) => new Promise((resolve, reject) => {
  axios.get(`https://zekais-api.herokuapp.com/jsholat?daerah=${daerah}&apikey=${zekais}`)
  .then(({ data }) => {
    const text = `Jadwal Sholat ${daerah}\n\nImsyak: ${data.result.imsak}\nSubuh: ${data.result.subuh}\nDzuhur: ${data.result.dzuhur}\nAshar: ${data.result.ashar}\nSunset: ${data.result.sunset}\nMaghrib: ${data.result.maghrib}\nIsya: ${data.result.isya}`
    resolve(text)
  })
  .catch((e) => reject(e))
})
...
const cuaca = async (url) => new Promise((resolve, reject) => {
    axios.get(`https://zekais-api.herokuapp.com/cuaca?daerah=${url}&apikey=${zekais}`)
        .then((res) => {
            if (res.status == 500) resolve('Maaf daerah kamu tidak tersedia')
            const text = `Cuaca di: ${res.data.Nama}\n\nCuaca: ${res.data.Cuaca}\nAngin: ${res.data.Angin}\nDesk: ${res.data.Keterangan}\nKelembapan: ${res.data.Kelembaban}\nSuhu: ${res.data.Suhu}\nUdara: ${res.data.Udara}`
            resolve(text)
        })
        .catch((err) => {
            reject(err)
...
const tulis = async (teks) => new Promise((resolve, reject) => {
    axios.get(`https://api.dapuhy.xyz/api/maker/nulis?text=${teks}&apikey=${dapuhyapi}`, {
...
const artinama = async (nama) => new Promise((resolve, reject) => {
    axios.get(`https://cakrayp.herokuapp.com/api/people?name=${nama}&apikey=${cakrayp}`)
        .then((res) => {
            resolve(res.data.result.meaning)
        })
        .catch((err) => {
            reject(err)
        })
...
const cekjodoh = async (nama, pasangan) => new Promise((resolve, reject) => {
    axios.get(`https://api.zeks.me/api/primbonjodoh?apikey=${apivinz}&nama1=${nama}&nama2=${pasangan}`)
        .then((res) => {
            const textc = `Nama: ${res.data.result.nama1}\nPasangan: ${res.data.result.nama2}\nPositif: ${res.data.result.positif}\nSisi Negatif: ${res.data.result.negatif}`
            resolve({ text: textc })
        })
        .catch((err) => {
            reject(err)
...
const covidindo = () => new Promise((resolve, reject) => {
    axios.get(`https://api.dapuhy.xyz/api/others/corona?negara=indonesia&apikey=${dapuhyapi}`)
        .then((res) => {
            const textv = `Info Covid-19\n\n*Total Kasus :* ${res.data.kasus}\n*Sembuh :* ${res.data.sembuh}\n*Meninggoy :* ${res.data.meninggal}\n*Sembuh :* ${res.data.sembuh}`
            resolve(textv)
        })
        .catch((err) => {
            reject(err)
...
const facebook = async (url) => new Promise((resolve, reject) => {
    axios.get(`https://zenzapi.xyz/downloader/facebook?url=${url}&apikey=${zenzapi}`)
    .then(res => {
        resolve(res.data)
    })
    .catch(err => {
